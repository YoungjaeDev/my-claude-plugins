...
- **Sync the `AGENTS.md` mirror counts too** — the Codex/Hermes-facing mirror carries its own count-homes that no `--check` guards: the Hermes-allowlist count (`현재 N개: …`) and the Codex verification comment (`# N entries`). A Hermes-eligible plugin add updated `CLAUDE.md`/`README.md` but left both `AGENTS.md` mirrors stale until a reviewer caught it. Update them in the same change as the `CLAUDE.md` Hermes/Codex-eligible bumps.
