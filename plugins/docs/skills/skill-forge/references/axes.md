...
What decides each axis when judging a skill. `docs:skill-audit` applies all seven to one skill; `docs:skill-fleet-review` runs the pointer axis over every skill in the tree and the remaining six over a measured cohort. The axes are the same in both — only the loop cardinality differs, which is why they live here instead of in either body.
