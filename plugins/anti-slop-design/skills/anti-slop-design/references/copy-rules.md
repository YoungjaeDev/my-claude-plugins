...
## 1. 핸드오프 전 체크리스트 (덱 8원칙 — 사용자 경험 기반, 한국어 카피 공통)
...
4. **register는 청중 함수** — 비개발자 청중 = 일상어("구축/산정/연동" -> "만들기/잡기/이어주기"); 학술/전문가 청중 = 정착 기술용어 유지(첫 등장 1회 한글 병기), 구어체 평탄화 금지. 발표 맥락(강연/학술/피치)에 톤앤매너를 맞춘다.
5. **제목은 담백하게, 설득은 박스에** — 제목에 영업·당위 꼬리표 금지.
6. **내부 표기·버전 라벨 노출 금지** — 내부 버전 태그(예: "v4"), 파이프라인 단계 은어(예: "intake"), 미확정 일정 추정치처럼 청중에게 무의미한 표기를 뺀다.
