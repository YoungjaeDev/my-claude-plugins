...
- [A detector must never convert "could not look" into "nothing wrong"](detector-cannot-look.md) — read-only checks (config detectors, review gates, convergence/merge loops) fail as false-clean via pipefail abort, `|| true`, degraded remote payloads, or a non-portable tool; guard on the exact question, require the full shape, give the gate test coverage, verify tool portability under `env -i PATH=/usr/bin:/bin` — but only when the stripped environment still holds every tool the script needs, since a jq-based suite run that way dies of tool absence having verified nothing (assert the tools under test are BSD builds instead) — and when you widen a signal's definition sweep every consumer of it — a sibling left on the narrow definition brings the same trap back as a false *block* instead of a false clean; and prefer `if cmd -v X …; then X …; fi` over `cmd -v X && X … || true`, whose `|| true` swallows the run failing as well as the tool being absent; and check a portability fix in both directions, because a single-platform lens hides both the platform-agnostic defect and the regression the fix just created (promoted_from [[detector-cannot-look-vs-nothing-wrong]]).
