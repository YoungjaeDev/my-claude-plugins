...
설치 후 `/dev:resolve-issue 123` 처럼 `/plugin:skill` 형태로 호출한다. 플러그인명을 짧게 둔 이유는 호출 길이와 가독성이다. 슬래시 메뉴는 `:`·`-` 를 무시하고 이름 안의 단어 시작에서도 매칭하므로 `/cr` 만 쳐도 `/dev:cr-fix` 가 하이라이트된다 (Claude Code 2.1.236 이상).
