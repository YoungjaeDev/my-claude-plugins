...
- `prompt_inject.sh` fires **every** prompt with a fixed ~6-line block: Korean-default response, a few core behavioral one-liners (surgical-diff, AskUserQuestion-first, no AI attribution, no emoji, verify-before-report), and a pointer to consult `.llmwiki/insight/` (promoted cross-agent rules) then the wiki MOC *before* reasoning. It does **not** inline insight/wiki content — only the instruction to read it. Claude reverts to English over long sessions, and neither agent is otherwise reminded to check the wiki/insight layer; this block fixes both. Zero deps (no `jq`/`python`) — JSON encoding is bash parameter expansion.
