...
reviews=$(gh api --paginate "repos/$OWNER/$REPO/pulls/$PR_NUM/reviews" 2>/dev/null \
  | jq -s 'add // []' \
  | jq --arg t "$PUSH_TIME" '[.[] | select((.user.login // "") | test("^coderabbitai(\\[bot\\])?$"; "i")) | select(.submitted_at > $t)] | length')
...
comments=$(gh api --paginate "repos/$OWNER/$REPO/issues/$PR_NUM/comments" 2>/dev/null \
  | jq -s 'add // []' \
  | jq --arg t "$PUSH_TIME" '[.[] | select((.user.login // "") | test("^coderabbitai(\\[bot\\])?$"; "i")) | select(.created_at > $t or .updated_at > $t)] | length')
