...
bridge_source: github-dev/commands/resolve-issue
---

# Resolve GitHub Issue

Act as an expert developer who systematically analyzes and resolves GitHub issues. Receive a GitHub issue number as argument and resolve the issue. Follow project guidelines in `@AGENTS.md`.
...
> **Note**: For parallel development, create worktrees manually before starting Claude sessions. See AGENTS.md for the recommended worktree workflow.
...
2. **Verify Plan File Alignment (If Exists)**:
   ...
   - Common patterns: `Plan: /path/to/plan.md`, `See: .codex/plans/xxx.md`
   - If plan file exists:
...
11.5. **Update Project Tracking State** (if milestone exists):
    ...
    - If milestone exists:
      ...
      2. Check for state file: `.codex/state/project-tracking-{slug}.json`
...
12. **[NEW] Cleanup**:
    - Archive state file to `.codex/state/archive/`
...
### State File Location
Sessions are saved to: `.codex/state/github-dev-{issue-number}.json`
...
```bash
mkdir -p .codex/state
cat > .codex/state/github-dev-${ISSUE_NUMBER}.json << 'EOF'
{... state JSON ...}
EOF
```
...
```bash
mkdir -p .codex/state/archive
mv .codex/state/github-dev-${ISSUE_NUMBER}.json \
   .codex/state/archive/github-dev-${ISSUE_NUMBER}-$(date +%Y%m%d).json
```
