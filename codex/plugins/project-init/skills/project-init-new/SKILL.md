...
bridge_source: project-init/commands/new
---
...
- **Minimal seeding, explicit follow-ups**: Day 1 에 진짜 필요한 것 (`.codex/` 빈 구조, AGENTS.md stub, AGENTS.md review guidelines, README/CHANGELOG, gh 레포) 만 시드. tech-stack 기반 rules 생성 (`$write-rules`) 와 wiki domain 인터뷰 (`$bootstrap-wiki`) 는 **호출하지 않고 Phase 7 안내만**. 빈 프로젝트에 generic 콘텐츠 생성하면 사용자가 덮어쓰는 비용 발생.
...
## Phase 2 — `.codex/` Scaffold (structure only)

```bash
bash ${CLAUDE_PLUGIN_ROOT}/scripts/idempotent-seed.sh ensure-claude-dirs
# 생성: .codex/{spec,rules}/.gitkeep + .llmwiki/{raw,wiki}/.gitkeep
```

`bootstrap-wiki` / `write-rules` 는 호출하지 않는다 — 빈 프로젝트에는 적을 lore 도, tech-stack signal 도 없다.

## Phase 3 — AGENTS.md Minimal Stub

기존 `AGENTS.md` 가 있으면 skip with notice. 없으면 다음 형태로 작성:
...
- **사용 순서**:
  1. lore 가 필요할 때 → `$query-wiki` 먼저
  2. 새 발견 → `$ingest-finding`
  3. PR merge 후 → `$post-merge` 가 자동으로 `$post-merge-wiki` 체이닝
- **현재 상태**: wiki 비어있음. 적극 채워라. 첫 도메인 lore 가 쌓이기 시작하면 `$bootstrap-wiki` 호출로 도메인 구조 인터뷰를 받는다.

## Setup Status

이 파일은 `/project-init:new` 가 만든 minimal stub 이다. 코드가 어느 정도 쌓이면 다음을 호출해라:

- `$write-rules` — tech-stack 기반 AGENTS.md + `.codex/rules/*.md` 재생성
- `$bootstrap-wiki` — 첫 wiki 도메인 인터뷰 + 템플릿 시드

> 사용자의 global `~/.codex/AGENTS.md` 가 항상 우선한다. 이 파일은 프로젝트 한정 규칙만 보관한다.
```
...
git add .codex/ .llmwiki/ AGENTS.md AGENTS.md README.md CHANGELOG.md
...
Files seeded:
  ...
  AGENTS.md       (minimal stub — LLM Wiki entrypoint)
...
Next actions (call when ready):
  1. 코드가 쌓이면        → $write-rules
     (tech-stack 기반 AGENTS.md + .codex/rules/*.md 재생성)

  2. 첫 도메인 lore 쌓이면 → $bootstrap-wiki
     (도메인 인터뷰 + .llmwiki/wiki/<domain>/ 구조 시드)

  3. 첫 PR merge 후        → $post-merge
     (자동으로 $post-merge-wiki 체이닝)
```
...
- 외부 boilerplate auto-download (cookiecutter 등 — 필요하면 `$scout` 별도 호출)
