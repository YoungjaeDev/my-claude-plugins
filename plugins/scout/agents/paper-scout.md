...
name: paper-scout
description: |
  Academic paper research scout. Wraps the `scout:paper-search` plugin's 8-source
  ...
  axis of scout's fan-out research pipeline. Invoked by
...
- Single-paper PDF download or full-text read → call `scout:paper-search` `download_*` / `read_*` directly; this scout is metadata-only
