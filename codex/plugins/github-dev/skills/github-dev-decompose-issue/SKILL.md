---
name: github-dev-decompose-issue
description: "Decompose Work"
bridge_source: github-dev/commands/decompose-issue
---

## Decompose Work

Break down large work items into manageable, independent issues. Follow project guidelines in `@AGENTS.md`.

## Workflow

1. Check issue numbers: Run `gh issue list` to view current issue numbers
2. **Discover available components**:
   - Scan `.codex/agents/` for custom agents (read YAML frontmatter)
...
9.5. **Define Architecture & Workflow Mapping** (for project progress tracking):
   ...
   - Save initial state file:
     ```bash
     mkdir -p .codex/state
     cat > .codex/state/project-tracking-${SLUG}.json << 'STATEEOF'
     {
       "version": "2.0.0",
       "milestoneId": null,
...
10. **Ask about GitHub creation**: Use AskUserQuestion to let user decide on milestone and issue creation
    ...
    - Update state file with milestoneId:
      ```bash
      STATE_FILE=".codex/state/project-tracking-${SLUG}.json"
...
11. **Add issues to GitHub Project (optional)**
   ...
   - If no project exists: Display "No project found. You can create one with `$init-project`" and skip
