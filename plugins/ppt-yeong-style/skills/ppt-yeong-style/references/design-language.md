...
6. **팝(pop)** — 크게 튀는 지점은 위계의 정점에만 고립된다(표지, 섹션 구분선, 큰 숫자 하나,
   ...
   아니다. **이 테스트를 통과하는 서사 신호(개념 메타포 스토리 일러스트)는 본문 장에도 허용된다** —
   표지·전환 격리는 *장식* 팝에만 적용된다(→ `images-and-pop.md` §5d).
...
- **pop 격리는 그대로** — *장식* 이미지 배경+타입 오버레이는 표지·전환 divider·마무리에만.
  본문 장은 미니멀(스크림·무드 배경 없음) — 단 서사 개념 일러스트는 예외(§5d, honesty test 통과).
