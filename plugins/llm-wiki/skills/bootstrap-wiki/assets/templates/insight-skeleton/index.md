...
The **insight layer** is the promoted, cross-agent-visible top of the knowledge system. It holds the small set of findings that earned graduation from `.llmwiki/wiki/`. Both Claude Code and Codex read it (it lives under the neutral `.llmwiki/` root), and the `core-config` `prompt_inject.sh` hook points every prompt here *first*, before the wiki MOC.
