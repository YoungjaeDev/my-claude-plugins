#!/usr/bin/env bash
# Smoke test — wiki cross-runtime PLUGIN_ROOT resolvers (issue #111 + #129 review).
...
# PLUGIN_ROOT is DEFINED and resolves in all 5 wiki skills (review #4).
set -u
HERE=$(cd "$(dirname "$0")" && pwd)
PLUGIN_DIR=$(cd "$HERE/.." && pwd)                 # plugins/wiki
...
# Codex plugin cache pointing wiki -> the real plugin tree (has both markers).
mkdir -p "$TMP/cache/mkt/wiki"; ln -s "$PLUGIN_DIR" "$TMP/cache/mkt/wiki/2.5.1"
...
[ "$OUT" = "$TMP/cache/mkt/wiki/2.5.1" ] && pass "bootstrap: Codex cache -> $OUT" || die "bootstrap Codex cache ($OUT)"

# Source tree
OUT=$(cd "$REPO_ROOT" && env -u CLAUDE_PLUGIN_ROOT CODEX_PLUGIN_CACHE="$TMP/none" bash "$BW")
[ "$OUT" = "plugins/wiki" ] && pass "bootstrap: source tree -> $OUT" || die "bootstrap source tree ($OUT)"
...
[ "$OUT" = "$TMP/cache/mkt/wiki/2.5.1" ] && pass "stale CLAUDE_PLUGIN_ROOT falls through to cache" || die "stale env should fall through ($OUT)"

# review #2 — an incomplete higher version (2.9.0, empty) must be skipped for the
# complete lower version (2.5.1).
mkdir -p "$TMP/cache2/mkt/wiki/2.9.0"; ln -s "$PLUGIN_DIR" "$TMP/cache2/mkt/wiki/2.5.1"
OUT=$(cd "$TMP/foreign" && env -u CLAUDE_PLUGIN_ROOT CODEX_PLUGIN_CACHE="$TMP/cache2" bash "$BW")
[ "$OUT" = "$TMP/cache2/mkt/wiki/2.5.1" ] && pass "incomplete 2.9.0 skipped, picked complete 2.5.1" || die "incomplete-version skip ($OUT)"
...
mkdir -p "$TMP/sp dir/mkt/wiki"; ln -s "$PLUGIN_DIR" "$TMP/sp dir/mkt/wiki/2.5.1"
OUT=$(cd "$TMP/foreign" && env -u CLAUDE_PLUGIN_ROOT CODEX_PLUGIN_CACHE="$TMP/sp dir" bash "$BW")
[ "$OUT" = "$TMP/sp dir/mkt/wiki/2.5.1" ] && pass "spaced cache path resolves" || die "spaced cache path ($OUT)"

# review #4 — PLUGIN_ROOT is DEFINED in all 5 wiki skills AND resolves to a
...
[ "$fail" = 0 ] && echo "wiki resolver smoke: ALL PASS" || { echo "wiki resolver smoke: FAILURES"; exit 1; }
