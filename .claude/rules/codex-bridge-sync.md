...
- **Require YAML frontmatter on every source SKILL.md.** `syncOne` runs `parseFrontmatter` first; a SKILL.md that opens with an H1 and has no `---` fences yields no frontmatter, so it is dropped with `status: 'skipped', reason: 'source-missing-frontmatter'` plus a stderr `... has no frontmatter, skipping` warning — it never reaches Codex. The `skipped: N` count in the dry-run report is the durable signal (the per-file warning is easy to miss). Author every skill with at least a `name`/`description` block. This skip reason is distinct from `non-managed-collision` (target-side) — `source-missing-frontmatter` is source-side.
