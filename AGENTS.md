...
- **P1 (should-block)** — plugin version/count drift (`plugin.json` ↔ `marketplace.json` ↔ AGENTS/README 트리·배지·`metadata.version`), idempotency 회귀 (재실행 시 사용자 파일 덮어쓰기·`nothing to commit` abort·`origin` 충돌), cross-platform shell 가정 (`sed -i` GNU-only / `${VAR,,}` Bash 4+), API 실패를 빈 결과로 삼키는 패턴 (`gh api ... || echo "[]"`), `Read`/`Edit` 영역을 `Bash cat`/`sed` 로 우회.
...
- 발견사항 제시 순서, 플러그인 추가/제거·skill 추가 체크리스트, `--slurp` 페이지네이션, sed substitution safety, 도입-버전 마커 예외, plugin-cache 이슈 등 **전문은 [`code_review.md`](code_review.md)**.
