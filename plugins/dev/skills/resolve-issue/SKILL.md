...
9.4. **[NEW] E2E suite (opt-in, auto-detected)**:
    ...
    - **If not detected**: silent skip. resolve-issue does not require the `dev:e2e-*` skills, a `playwright.config`, or any E2E setup to exist — it works whether or not E2E is configured (loose coupling).
...
Session state enables workflow recovery after interruption. Sessions are saved to `.claude/state/github-dev-{issue-number}.json`, checkpointed after each phase and archived to `.claude/state/archive/` on success. Schema, checkpoint-save and cleanup blocks: `references/state-management.md`.

---

## Verification Gates

Quality gates that must pass before commit. BUILD and TEST are required and block the commit on failure; LINT and TYPE_CHECK warn only (LINT blocks under `--strict`). Check types, project-type detection table and the verification agent prompt: `references/verification-and-review.md`.

---

## 2-Stage Review Protocol

Before PR creation, implementation passes Spec Compliance, then Code Quality review, each with at most 3 retries before escalating to the user; `--skip-review` skips both. Agent prompts and the review loop: `references/verification-and-review.md`.
