...
The capture side has its own version, with no `|| true` anywhere: **the output went to the stream you did not read.** `check-skill-prose.mjs` emits every line — warnings, the all-clear, and the abort notice — through `console.error`, so `node scripts/check-skill-prose.mjs | wc -l` is `0` while `2>&1 | wc -l` is `24`. A fleet report whose contract demands the guard's output *verbatim*, implemented as a plain stdout capture, rendered a correctly-fenced section containing nothing, and the surrounding prose still asserted "23 warnings" from a separate reading. Nothing errored, exit was 0, and the empty block reads as a formatting quirk rather than a lost input. When a contract says "carry this output", the capture must name the streams (`2>&1`) and then **assert on the captured text** before it is embedded — the guard's own summary line is a cheap thing to require.
...
The mode has a second, worse damage direction, seen when the same blind spot feeds a *removal* decision rather than a parity claim. A doc-pruning pass verified that eight design documents had no inbound references by running `rg -l --fixed-strings "$f" .` per file — 0 hits for all eight, deletion authorized. `.llmwiki/` is a dot-directory, so the check never opened the wiki, and `plugin-ops/skill-authoring-source-grounded-then-audit.md` (`status: active`) was citing two of them as `> Evidence:`. The reviewer caught it on the PR; `rg --hidden` surfaces both instantly. Note the asymmetry: in the parity case the blind spot lets something stale *survive*, which is recoverable at any later date. Pointed at a deletion it destroys a live citation and the damage is only visible to whoever next follows that link. **A grep that gates a removal needs `--hidden` more than a grep that gates a parity claim, not less.**
...
**The same shape outside portability: a proxy question answered confidently about the wrong consumer.** An over-engineering audit of `cr-fix` asked "which of `parse-args.sh`'s exported variables does another *script* read?", found none for seven of them, and reported 435 lines of dead flag-parsing — the highest-confidence, pure-deletion item on its list. Every one of the fourteen is read, in `SKILL.md`, some eighteen times. In a skill the dispatcher is the model reading the body, so scripts-reading-scripts was never where the consumption happens; the audit measured a proxy for the consumer and reported the proxy's silence as absence. The tell is available before the work: **name the consumer of the thing you are about to call dead, then check that your query actually reaches it.** Three sibling claims from the same audit (phantom flags, zero-call-site scripts) failed the same way, and all three looked like minus-only diffs.
...
14. **PR #206** (`chore: 스킬 전수 검토 + README 상세 절 재구성 + 죽은 문서 정리`) — Mode 6's damage-direction half and a Mode 9 instance outside portability. The inbound-reference check that authorized deleting eight design documents ran `rg -l --fixed-strings "$f" .` and returned 0 hits for every file; `.llmwiki/` is a dot-directory, so an `status: active` page citing two of them as `> Evidence:` was never seen. Caught by the Codex reviewer on the PR, reproduced by re-running with `--hidden`. The same PR's over-engineering audit reported 7 dead exported variables in `cr-fix/scripts/parse-args.sh` by asking which *other scripts* read them; all fourteen are read in `SKILL.md`, because a skill's dispatcher is the model reading the body.
15. **`skill-fleet-review`'s first sweep (2026-08-11, PR #206)** — the capture-side instance under Mode 2. `check-skill-prose.mjs` writes all output via `console.error`; a stdout-only capture embedded an empty verbatim block in the audit report while the prose around it asserted 23 warnings. Measured: `| wc -l` → 0, `2>&1 | wc -l` → 24. The reviewer flagged the summarized block, not the empty one.
