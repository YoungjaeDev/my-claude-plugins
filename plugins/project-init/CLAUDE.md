...
- **wiring does not diagnose another owner's territory**: wiki-page health is owned by `/llm-wiki:lint-wiki`, and mem0 store/config posture by `/mem0-ops:fleet-scan`. wiring looks only at filesystem signals — "does a wiki exist / is the layout right / has mid-drain capture piled up". Overlap means the day comes when two diagnostics give different answers.
