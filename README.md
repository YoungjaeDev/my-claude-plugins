...
3. **wiki ingest** — 재사용될 lore(결정의 근거, 제약조건, 도메인 사실, 반복될 판단 기준)만 추려 `llm-wiki:ingest-finding` 에 넘깁니다. 회의록 통째로는 넘기지 않습니다. 단발 액션아이템·일정은 digest 에만 남습니다. wiki 루트나 `ingest-finding` 이 없으면 사유 한 줄을 남기고 skip 하며, 스킬을 실패시키지 않습니다.
