...
- Issue the remaining `codex exec` calls (N, or N-1 after an anchor) as parallel Bash tool calls, each writing its own file. Every variant session carries `count: 1` — the variation lives in the prompts, so a session that generates the full `-n` count would bill N times the images asked for. Each session also gets its own filename base; parallel sessions sharing one base overwrite each other and the run ends with fewer images than variants. The distinguishing suffix belongs to the filename only, never to the creative prompt.
