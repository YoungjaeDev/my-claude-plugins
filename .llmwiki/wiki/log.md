...
- plugin-ops/detector-cannot-look-vs-nothing-wrong.md: **Mode 6 gains its damage-direction half.** The recorded instance was a parity gate whose 0-hit reading let a stale string *survive*. This run inverted it — the same dot-dir blind spot returned a clean inbound-reference check that *authorized a deletion*, and the deleted files were cited by an `status: active` wiki page. A blind spot that silently keeps something is annoying; the same blind spot pointed at a removal decision destroys a live reference. **Mode 9 gains an instance**: an over-engineering audit measured whether other *scripts* read `parse-args.sh`'s exported variables, concluded 7 were dead, and was wrong on all of them — cr-fix's dispatcher is the LLM reading `SKILL.md`, so the question "does another script read this" was never the question. sources 13 -> 14; last_verified 2026-08-10 -> 2026-08-11.
- llm-wiki-design/deleted-subject-not-stale.md: adds the deletion-side rule the page implied but never stated — when pruning documents whose subject was removed, the test is **"the subject no longer exists", never "nothing links to it"**. The link-absence heuristic eats a freshly written audit report (orphan by construction the moment it is created) and a live plugin's design record, while missing exactly the dangling-citation case this page exists for. sources 3 -> 4; last_verified 2026-08-08 -> 2026-08-11.
...
## 2026-07-27 — enforcement-surface blind spot in a removal recommender (ingest-finding, post-merge #167)

- plugin-ops/detector-cannot-look-vs-nothing-wrong.md: added **Mode 7** — a diagnostic that *recommends deletions* checked the documented enforcement surfaces (`.pre-commit-config.yaml`, lint/format configs) but not the one this repo actually uses (`core.hooksPath` → `.githooks/`), and emitted "safe to cut" for a doc block a hook mechanically asserts. Sibling of Mode 4's "reading the wrong surface entirely", but the uninspected axis is *what enforces the target*, not *where the target lives*. New source 6 (PR #167). last_verified 2026-07-24 -> 2026-07-27, sources 5 -> 6.
- index.md: extended the detector hook with the Mode 7 clause.
- routing dedup: the repo-specific half of this finding (run `check-doc-consistency.mjs`; it guards the README tree + AGENTS `## Plugins` table + count strings, and blocks at pre-commit) landed in `AGENTS.md` `## 검증` in this same post-merge run (Step 6) — not re-recorded here. The wiki keeps only the generalizable invariant.
- insight graduation: declined — the parent invariant already sits at `.llmwiki/insight/detector-cannot-look.md`, whose rule statement ("a check that cannot evaluate an axis must say so and never emit an all-clear for the part it could not inspect") covers this shape verbatim. A first-occurrence variant fails the recurs-across-sessions criterion, and the insight's `promoted_from` already points at this page for the evidence.

## 2026-07-28 — a widened signal that reached one consumer, not all (ingest-finding, post-merge #183)

- plugin-ops/detector-cannot-look-vs-nothing-wrong.md: added **Mode 8** — the fix for "reading the wrong surface" was applied to one consumer of a signal and not to its siblings, so the same trap resurfaced a third time in the same skill (`engagement-gate.sh` counted only `created_at` while `sniff-cr-rate-limit.sh` had already been widened to `created_at or updated_at`); CodeRabbit edits its walkthrough in place on a clean re-review, so a genuine convergence read as "CR never looked" and `--auto-merge` could never fire on the normal path. Also extended **Mode 5** past detectors: the same non-portable-tool trap in a *producer* is swallowed by pipeline *position* rather than by `set -e` (`$(... | md5sum | cut | head)` exits 0 from `head`), and `|| true` on the watchdog call erased the very exit status the guarding test asserted. New source 7 (PR #183). last_verified 2026-07-27 -> 2026-07-28, sources 6 -> 7.
- index.md: extended the detector hook with the Mode 8 clause.
- insight/detector-cannot-look.md: added the consumer-sweep bullet (widening a signal is not done until every reader of that signal is swept) and the producer/pipeline-position variant. evidence_count 4 -> 5, last_verified 2026-07-22 -> 2026-07-28.
- insight/index.md: extended the detector hook with the consumer-sweep clause.
- routing dedup: the mechanical half of this finding — the GNU-only construct list gaining `md5sum`/`date -d`/`stat -c`/`timeout`, the "exit status disappears mid-pipeline / `|| true` on an asserted status" P1, and the write-side twin of the API-failure rule (never `gh issue/pr edit --body` an unchecked transform result) — landed in `code_review.md` P1 + the `AGENTS.md` mirror in this same post-merge run (Step 6). The wiki keeps only the generalizable invariant.

## 2026-07-28 — a guard whose absorption is wider than its intent (ingest-finding, post-merge #185)

- plugin-ops/detector-cannot-look-vs-nothing-wrong.md: extended **Mode 2** — `|| true` is usually discussed as "patching an abort makes it invisible", but the sharper form is *scope*: `cmd -v X && X … || true` absorbs the whole `&&` chain, so "X is not installed" (the intended no-op) and "X ran and failed" (a real breakage) both exit 0. Measured on the shipped hook command: script exiting 3 read as rc 0 before, rc 3 after rewriting to `if cmd -v X …; then X …; fi`. Notable provenance: the rule that caught it (`code_review.md` P1, "do not put `|| true` where the exit status is the signal") was added by PR #183 two merges earlier, and the Codex reviewer cited that exact `AGENTS.md` line back at the author. last_verified 2026-07-28, sources 7 -> 8.
- index.md: extended the detector hook with the guard-scope clause.
- insight/detector-cannot-look.md: sharpened the `|| true` bullet to name the scope trap rather than only the false-clean symptom. last_verified 2026-07-28.
- routing dedup: the mechanical half (use `if …; then …; fi` so the guard's absorption is limited to the condition) landed in `code_review.md` P1 + the `AGENTS.md` mirror in this same post-merge run (Step 6). The wiki keeps the invariant and the provenance.
- not ingested: the `${CLAUDE_PLUGIN_ROOT}` quoting asymmetry (Claude manifests unquoted while the paired Codex descriptors were already quoted) is the same unswept-consumer shape Mode 8 documented yesterday from PR #183; recorded as a second occurrence in that mode's text rather than as a new mode.

## 2026-07-28 — the audit lens is itself a blind spot (ingest-finding, post-merge #186)

- plugin-ops/detector-cannot-look-vs-nothing-wrong.md: added **Mode 9** — a single-platform portability audit cannot see two whole classes. (a) A defect that breaks identically everywhere never answers "what breaks on BSD?": the project-init cache resolver sorted whole paths, so the marketplace directory name outranked the version and `sort -V` picked `zeta/0.4.0` over `alpha/0.10.0` on GNU too; the audit found only the lexicographic-fallback half. (b) The fix's reverse regression is invisible to the same question: `python` -> `python3` is right for macOS 12.3+ and wrong for a python.org Windows install, which ships `python` and the `py` launcher but no `python3` — caught twice by the Codex reviewer, not by the audit. Corollary: a severity rating derived inside that lens does not transfer — this call site was rated cosmetic because "macOS has no CUDA so it exits anyway", which is false on Windows where CUDA exists and the feature is real. New source 9 (PR #186). last_verified 2026-07-28, sources 8 -> 9.
- index.md: extended the detector hook with the Mode 9 clause.
- insight/detector-cannot-look.md: added the audit-lens bullet (a check scoped to one platform cannot report on the platforms it did not ask about, including the one it just broke). evidence_count 5 -> 6, last_verified 2026-07-28.
- insight/index.md: extended the detector hook with the both-directions clause.
- routing dedup: the mechanical half (verify a portability fix in both directions; detect the interpreter instead of pinning a name; do not trust a severity rating derived inside a single-platform lens) landed in `code_review.md` P1 + the `AGENTS.md` mirror in this same post-merge run (Step 6). The wiki keeps the generalizable invariant about the lens.
- also fixed in Step 6, not lore: `plugins/project-init/CLAUDE.md` carried the pre-PR-#186 resolver snippet verbatim, so the plugin's own doc showed code the plugin no longer runs. Same code-vs-doc drift shape the Codex reviewer flagged on PR #185, caught here by post-merge rather than by a reviewer.

## 2026-07-29 — the stock-userland rule has a floor, and a guard leaks through its own exemption (ingest-finding, post-merge #187)

- plugin-ops/stock-userland-verification.md: qualified **The rule** — `env -i PATH=/usr/bin:/bin` is sound only when every tool the script needs is in stock userland. The cr-fix suite is built on `jq`, which on macOS lives in the Homebrew prefix, so that form kills the suite on tool absence having verified nothing; on Linux `/usr/bin/jq` exists and it passes, so a local check confirms the wrong thing. Recorded the replacement discipline for that case (keep PATH, assert the tools under test are BSD builds, fail loudly when Homebrew coreutils shadow them, invoke `/bin/bash` not `bash`). New source 4 (PR #187). last_verified 2026-07-29, sources 3 -> 4.
- plugin-ops/detector-cannot-look-vs-nothing-wrong.md: added **Mode 10** — a guard leaks through its own exemption. `check-shell-portability.mjs` exempts a GNU-only token when something nearby makes it portable; the first version counted a *comment* as that something, and a comment mentioning `cksum` sat above the fixed `md5sum` line, so the guard passed the exact defect it was built to catch. Same shape in the `sed -i` exclusion: "followed by a quote" was meant to admit BSD's `sed -i ''` and silently admitted GNU `sed -i "s/a/b/"`. New source 10 (PR #187). last_verified 2026-07-29, sources 9 -> 10.
- index.md: extended the stock-userland hook with the tool-absence floor, and the detector hook with the Mode 10 clause.
- insight/detector-cannot-look.md: qualified the two unconditional `env -i PATH=/usr/bin:/bin` prescriptions (Mode-4 bullet + When-to-apply) — the stripped environment must still contain the tools the script needs, else the run proves nothing. evidence_count 6 -> 7, last_verified 2026-07-29.
- insight/index.md: extended the detector hook with the same qualification.
- routing dedup: the mechanical half (what `check-shell-portability.mjs` catches and passes, `# portability-ok:` as the escape hatch, and the macOS leg's no-`env -i` / assert-BSD / `/bin/bash` wiring) landed in `AGENTS.md` and `README.md` inside PR #187 itself, so Step 6 added nothing. The wiki keeps the generalizable invariants: the stock-userland rule's floor, and the exemption-leak mode.
- not ingested: `docs/audit/measure-skills.mjs` dropped its `readdirSync`-order dependence (dotfile filter + content-byte tiebreak, because `lines` ties held filesystem order and ext4 != APFS, while `localeCompare` would only trade filesystem dependence for ICU dependence). Real and cross-platform, but one occurrence with no existing page whose scope it refines — a new page for a single nugget is the naive-accumulation rot mode. Recorded here; it graduates if it recurs.

## 2026-07-30 — a run's own housekeeping can park your work where you will not look (ingest-finding, post-merge #195)

- plugin-ops/worktree-squash-merge-gotchas.md: added **gotcha 6** — `post-merge` Step 2 stashes uncommitted tracked changes and Step 3 immediately checks out the base branch, so the stash is labelled with a branch the working tree has left. The parked file then reads at its base-branch content and `git status` is clean, which is indistinguishable from "the edit was never made". Same shape as gotcha 1 (work exists, not where you are looking), reached by a different route. Evidence: `CLAUDE.md.global.ko`.
- plugin-ops/worktree-squash-merge-gotchas.md: extended **gotcha 2** with the read-only pre-check — `git merge-tree --write-tree --name-only <branch> <base>` names the conflicting files without touching the working tree or index, so the resolution can be planned before the merge rather than diagnosed after it. Evidence: `AGENTS.md`.
- plugin-ops/worktree-squash-merge-gotchas.md: fixed two self-description defects the page had accumulated — the intro still said "Four gotchas" over five sections, and `## Sources` numbered two entries "3". last_verified 2026-07-30, sources 4 -> 5.
- index.md: extended the worktree hook with the gotcha-6 clause and the merge-tree pre-check.
- routing dedup: nothing from PR #195's own diff became lore. Its config-side facts (the global file carries no sync notice, the plugin table keeps names and categories only, `dual-integration.md` is `paths:`-scoped) all landed as rules inside the PR itself, so Step 6 added nothing. The wiki keeps only the two process findings above, which the diff does not show.
- not graduated to insight: gotcha 6 is one observation. It meets generalizable and costly-to-violate, but not "recurs across sessions" — it graduates if a second run hits it.

## 2026-08-02 — an opt-in drift check is also a permission switch (ingest-finding, post-merge #197)

- llm-wiki-design/optin-drift-check-permission-switch.md: new page, status: active, volatility: stable, sources: 2 — `lint-wiki` Step 11 hashes only files whose frontmatter declares `sha256:`, so *omitting* the field marks a file as hand-editable. plaud-note-taking v0.2.0 uses that to keep a mutable derivation layer (`derived/`) inside `.llmwiki/raw/` without breaking raw immutability, with no new rule and no lint exemption. Also records the measured limit: the reverse guarantee is one-directional, since a hand-dropped `.txt` usually declares nothing and is skipped too.
- cr-fix-ops/cr-cli-fallback-contract-drift.md: added **4. The wrapper's completion signal can be read before it exists** — the background-task notification for `cr-cli-spawn.sh` fired "completed (exit 0)" while the marker file was still 0 bytes and the `coderabbit` child was still emitting heartbeats. Read at that instant, Step 7d's `emitted_complete=false` branch declares `cli_failed` and stops a healthy loop. New source 6 (PR #197). last_verified 2026-08-02, sources 5 -> 6.
- index.md: added the new llm-wiki-design hook; extended the CLI-fallback hook with the early-read clause.
- routing dedup: nothing from PR #197's own diff became config. The PILOTS/BASELINE count sentence was corrected inside the PR itself, and the rest of what the PR changed is plugin-internal discipline that lives in the skill body and its templates, so Step 6 added nothing.
- not graduated to insight: both findings are single observations. The permission-switch pattern meets generalizable and costly-to-violate but not "recurs across sessions"; it graduates if a second plugin reaches for it.
- not ingested: `[정정]` is a corrected *span*, not a *claim*, so it never earns a decision line on its own — a real precision defect that two reviewers disagreed about across iterations. It is plaud-internal discipline with no existing page whose scope it refines, and one nugget does not justify a page.

## 2026-08-02 — a guard that hand-rolls a parser + 38-marker staging drain (ingest-finding)

- plugin-ops/detector-cannot-look-vs-nothing-wrong.md: added **Mode 11** — the guard hand-rolls a parser of a real grammar. `check-shell-portability.mjs` took 4 review rounds and 16+ findings because every fix modelled one more shell construct and the next reviewer found one it did not; the leak is structural, not a run of oversights. New source 12 (session b3022a01, 2026-07-29). last_verified 2026-08-02, sources 11 -> 12.
- plugin-ops/council-cross-vendor-design.md: added the invocation surface to **Runtime scope** — the seats call the `codex` / `agy` **binaries** directly from the skill's own shell, never through the `codex` or `antigravity` plugins. Verified: `rg 'codex:rescue|agy-rescue|antigravity|Skill\('` over `plugins/council/` returns one hit, and it is an agy log path, not a plugin call. New source 3 (session 98f375db, 2026-07-29). last_verified 2026-08-02, sources 2 -> 3.
- index.md: extended the detector hook with the Mode 11 clause and the council hook with the direct-binary clause.
- **38 staging markers consumed** (`.llmwiki/.staging/pending-*.md`, captured 2026-07-27 .. 2026-08-02), triaged in three classes:
  - **32 subagent markers dropped as pre-synthesis slices.** 30 belong to one workflow run (`wf_6ea80663-6a0`, session b3022a01) whose fan-out agents were read-only macOS/BSD portability auditors plus adversarial verifiers; that audit's synthesized conclusions already landed via post-merge #187 (log entry 2026-07-29). 2 belong to session d3a04167, whose work became PR #197 (log entry above). A fan-out slice is not an independent source — ingesting each one is the naive-accumulation rot mode by construction.
  - **8 main-session markers already covered.** e0839716 / 57410504 / 5284adcf / 1cd729d7 are post-merge runs for #167 / #187 / #191 / #195, each of which already ran its own mandatory Step 8 ingest; dea8b704 is the previous 35-marker drain; d3a04167 + f1cd31db are PR #197 and this drain. 98834cfb and a7b7c12d are `CLAUDE.md.global` restructuring/mirror-verification work whose rules landed in the file itself.
  - **2 yielded the lore above**; 1 (e9873b90) was dropped as lore about `ppt-yeong-style`, a plugin since removed from the marketplace.
- the recurring shape worth naming: the Stop-hook capture and post-merge Step 8 are two capture paths over the same sessions, so a drain finds mostly already-ingested work. That is the system behaving correctly (over-capture is safe by design, per capture-curation-split), not a backlog of missed lore — but it means a drain's real output is a triage record, and the count of markers is not a count of findings.
- not graduated to insight: Mode 11 is one instance; the parent page is already the consolidated home for this family and does not itself meet the recurs-across-sessions bar as a promoted rule.

## 2026-08-02 — stale-window re-verification found two wrong facts, not two stale dates (ingest-finding)

Both pages had passed their 30-day `volatile` window. Re-checking against source (rather than bumping the date) contradicted a stated claim on each.

- e2e-harness-ops/playwright-ai-harness.md: re-ran `init-agents` + `trace --help` on **Playwright 1.62.1** (page was pinned 1.61.0). Corrected the `--loop` claim: the page asserted "there is no `copilot` value", and 1.62.1 offers `claude | codex | copilot | opencode | vscode | vscode-legacy` — `copilot` now exists and `vscode-legacy` is new. Added the `trace attachment <id>` subcommand (page listed only the plural `attachments`). Everything else re-confirmed byte-for-byte: prefixed agent filenames, `.mcp.json` generated with the same `playwright-test` server, root `seed.spec.ts` + `specs/README.md`, and no `playwright.config.*`. Pin moved to 1.62.1. New source 3. last_verified 2026-08-02, sources 2 -> 3.
- tally-form-ops/tally-api-schema-vs-live.md: re-checked the pricing half against `tally.so/pricing`. **Pro is $24/mo, not $29**, and a third **Business tier ($74/mo)** exists that the page did not record; the Pro feature list (custom CSS, remove branding, custom domain, email notifications, partial submissions) re-confirmed. The schema-vs-live divergence half was **not** re-verified — it requires live `POST /forms` and no `TALLY_API_KEY` was available — so the body now carries a dated scope line saying which half this pass covered. New source 5. last_verified 2026-08-02, sources 4 -> 5.
- index.md: extended the playwright hook with the `--loop` correction; the tally hook needed no change (it does not restate the price).
- the lesson the pass itself produced: a stale-window hint is a prompt to **re-check**, and re-checking is not bumping a date. Two of two pages examined this way were wrong in a specific, correctable way, and neither error was visible from the page alone — one needed the tool re-run, the other needed the vendor page. A page whose date is bumped without that work is worse than one left visibly stale, because the alarm that would have caught it is now silent.
