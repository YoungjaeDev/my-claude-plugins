<!--
Template: Root AGENTS.md skeleton (target ≤200 lines).

Use: write-rules skill Read s this when generating a NEW project's
root AGENTS.md or when TIGHTEN / SPLIT mode rebuilds it.
...
Design notes:
- No @import directives. `.codex/rules/*.md` auto-load handles it.
...
Modular rules live under `.codex/rules/` and load automatically when
working with matching files. This list is for human navigation only —
no `@import` needed since `.codex/rules/*.md` is auto-discovered.
...
- Path-scoped rules under `.codex/rules/` apply when editing files
  that match their `paths:` glob.
- {{Project-specific guidance for AI assistance.}}

<!--
End of root AGENTS.md skeleton.
...
.codex/rules/code-structure.md (no paths: frontmatter — always load).
-->
