...
Sync OMC plugin skills (`plugins/*/skills/**/SKILL.md`) AND commands (`plugins/*/commands/*.md`) to Codex CLI's native skill directory (`~/.agents/skills/`) idempotently. OMC is single source of truth (SSOT); the target is a derived artifact rebuilt every run.
...
- **Inject guidelines into `~/.codex/AGENTS.md` between `<!-- omc-guidelines:start -->` and `<!-- omc-guidelines:end -->` markers.** `plugins/*/guidelines/*.md` bodies run through standard transforms, are concatenated, and appended at end-of-file. The OMX marker region (`<!-- omx:generated:agents-md -->`) is never touched; our block must come AFTER OMX content. Idempotent (replace same marker pair on re-sync; append if absent).
- **Emit size warning when final AGENTS.md would exceed `config.agentsInject.warnBytes`** (default 32768 = Codex `project_doc_max_bytes` default). Users hitting the warning should raise the Codex-side knob in `~/.codex/config.toml` rather than truncating guidelines — the loader, not the model, is the bottleneck.
