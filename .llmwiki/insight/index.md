...
- [A detector must never convert "could not look" into "nothing wrong"](detector-cannot-look.md) — read-only checks (config detectors, review gates, convergence/merge loops) fail as false-clean via pipefail abort, `|| true`, degraded remote payloads, or a non-portable tool; guard on the exact question, require the full shape, give the gate test coverage, verify tool portability under `env -i PATH=/usr/bin:/bin`, and when you widen a signal's definition sweep every consumer of it — a sibling left on the narrow definition brings the same trap back as a false *block* instead of a false clean (promoted_from [[detector-cannot-look-vs-nothing-wrong]]).
