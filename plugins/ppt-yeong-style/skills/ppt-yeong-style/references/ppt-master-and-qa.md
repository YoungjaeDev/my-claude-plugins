...
- **PPTX 템플릿 파일을 그대로 채워달라는 요청(라우팅 경계)**: 사용자가 실제 .pptx 파일을 갖고 와서 "이 틀에 채워줘"라고 하면 ppt-master가 `template-fill-pptx` 경로(기존 슬라이드 틀 유지, 내용만 교체)로 빠진다. 이 경로에선 시각 규칙(색·구조·무드 락, mode/visual_style, spec_lock 기반 SVG 파이프라인)이 적용되지 않는다 — 템플릿이 이미 그 결정을 담고 있다. 다만 **글 관련 규칙(§3 원칙 15종·§7 윤문·§8b 스토리 흐름 review)은 형식과 무관하게 그대로 적용**한다(단, §8b "레이아웃 분포"는 기존 템플릿의 레이아웃을 그대로 쓰는 경로라 예외 — 위 §8b 항목 참조).
- **"이미 만든 PPT를 다듬어달라"(라우팅 경계)**: 페이지 수·순서·문구를 그대로 유지하는 `beautify-pptx` 경로도 마찬가지로 §2 md 소스 작성 규약 자체는 적용되지 않는다. 엔진에는 beautify 외에 native-enhance 계열 경로도 있다 — 어느 쪽을 탈지는 ppt-master 라우팅이 정하고, 글 규칙(§3 원칙 15종·§7 윤문·§8b 스토리 흐름, 레이아웃 분포 제외)은 어느 경로든 그대로 적용한다.
