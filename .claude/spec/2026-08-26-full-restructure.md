...
- [ ] 문서 스타일 전수 패스 (3단계, 통합 확정 후): 살아남는 플러그인 CLAUDE.md + SKILL.md (현재 기준 67개)를 sonnet 병렬 에이전트로 아래 스타일 규약에 맞춰 검사·수정. 규칙 집합과 스킬 의미는 보존, 표현만 재작성. references/는 온디맨드 로드라 제외, 루트 문서는 제로베이스 재작성이 대체.

  > 스타일 규약 (2026-08-26 사용자 지정, 원문 유지): 문서를 exhaustive knowledge dump로 작성하지 않는다. 정보를 선별(curate)하고 중요도에 따라 계층화한다. 하나의 항목에는 하나의 핵심 주장만 둔다. 예외·반례·구현 세부사항은 핵심 판단에 필요한 경우에만 포함한다. 모든 발견사항을 보존하려 하지 말고, 독자의 판단과 행동에 기여하지 않는 정보는 제거한다. 표현은 추상적·비유적·암시적으로 쓰지 말고, 구체적(concrete)·직설적(direct/literal)·명시적(explicit)으로 쓴다. 개념이나 수사보다 실제 대상·행동·조건·결과를 명확히 적고, 모호한 표현보다 구체적인 사실·수치·예시를 우선한다. (code_review.md는 2026-08-26 PR #211에서 이 규약으로 선행 재작성됨 — 레퍼런스 사례)
