...
CLAUDE.md placement, .claude/rules/ semantics, auto-memory, AGENTS.md
...
## CLAUDE.md 파일을 어디에 둘지

| 범위 | 위치 | 목적 |
|---|---|---|
| 관리 정책 | OS 별 시스템 경로 | IT/DevOps 조직 전체 지침 |
| **프로젝트** | `./CLAUDE.md` 또는 `./.claude/CLAUDE.md` | 팀 공유 |
| 사용자 | `~/.claude/CLAUDE.md` | 개인 선호도 (모든 프로젝트) |
| 로컬 | `./CLAUDE.local.md` | 개인 프로젝트별 (`.gitignore` 권장) |

작업 디렉토리 위의 디렉토리 계층의 CLAUDE.md 와 CLAUDE.local.md 는 시작 시 전체 로드된다. 하위 디렉토리 파일은 Claude 가 해당 디렉토리 파일을 읽을 때 필요에 따라 로드.

## 효과적인 지침 작성

CLAUDE.md 는 모든 세션 시작 시 컨텍스트 윈도우에 로드되어 대화와 함께 토큰을 소비한다.
...
CLAUDE.md 는 `@path/to/import` 구문으로 다른 파일을 가져올 수 있다. 가져온 파일은 확장되어 시작 시 컨텍스트에 로드된다.
...
**중요**: `@import` 된 파일도 시작 시 컨텍스트에 전체 로드됨. 토큰 절약 효과 없음 — 단순히 파일을 조직적으로 분할하는 도구. 토큰 절약이 목적이면 [`.claude/rules/`](#claude-rules-로-규칙-구성) 의 path-scoped 규칙을 사용할 것.

## AGENTS.md

Claude Code 는 `CLAUDE.md` 를 읽고 `AGENTS.md` 는 읽지 않는다. 저장소가 이미 다른 코딩 에이전트 (Codex / Cursor / Aider 등) 에 `AGENTS.md` 를 사용 중이면 `CLAUDE.md` 를 만들어 import 하는 패턴이 권장:
...
`/init` 은 기존 `AGENTS.md` / `.cursorrules` / `.windsurfrules` 등을 읽고 생성된 `CLAUDE.md` 에 통합한다.

## `.claude/rules/` 로 규칙 구성

대규모 프로젝트의 경우 `.claude/rules/` 디렉토리로 지침을 여러 파일로 구성. 지침이 모듈식이 되고 팀 유지보수가 쉬워진다. 규칙을 **특정 파일 경로로 범위 지정** 가능 — Claude 가 일치하는 파일로 작업할 때만 컨텍스트에 로드되어 노이즈와 토큰 둘 다 절약.
...
`paths` frontmatter 가 없는 규칙은 `.claude/CLAUDE.md` 와 동일한 우선순위로 시작 시 로드.
...
`~/.claude/rules/` 의 개인 규칙은 모든 프로젝트에 적용. 사용자 수준 규칙은 프로젝트 규칙 전에 로드되어 프로젝트 규칙에 더 높은 우선순위 부여.

## CLAUDE.md 파일 로드 순서

Claude Code 는 현재 작업 디렉토리에서 디렉토리 트리를 올라가며 각 디렉토리 CLAUDE.md 와 CLAUDE.local.md 를 읽는다. `foo/bar/` 에서 실행 시:
...
하위 디렉토리의 CLAUDE.md 는 시작 시 로드 안 되고, Claude 가 해당 하위 디렉토리 파일을 읽을 때 포함.
...
- **저장 위치**: 프로젝트당 `~/.claude/projects/<project>/memory/`
...
`/memory` 명령은 현재 세션에 로드된 모든 CLAUDE.md / CLAUDE.local.md / 규칙 파일 나열 + 자동 메모리 폴더 링크. 모든 파일은 일반 마크다운이라 직접 편집 가능.

## 모노레포: 특정 CLAUDE.md 파일 제외

대규모 모노레포에서 상위 CLAUDE.md 파일에 작업과 무관한 지침이 포함될 수 있다. `claudeMdExcludes` 설정으로 경로 또는 glob 패턴으로 특정 파일 건너뛰기:
...
`.claude/settings.local.json` 에 추가하면 로컬로 유지. 절대 파일 경로에 glob 매칭. 설정 레이어 (사용자 / 프로젝트 / 로컬 / 관리 정책) 전체에서 배열 병합.

## 메모리 문제 해결

### Claude 가 CLAUDE.md 를 따르지 않음

CLAUDE.md 콘텐츠는 시스템 프롬프트의 일부가 아니라 시스템 프롬프트 후 사용자 메시지로 전달. Claude 는 읽고 따르려 하지만 엄격한 준수 보장 안 됨.

디버깅:

1. `/memory` 실행해 CLAUDE.md 와 CLAUDE.local.md 가 로드되는지 확인
2. CLAUDE.md 가 세션에 대해 로드되는 위치에 있는지 확인
3. 지침을 더 구체적으로 ("2칸 들여쓰기" > "코드를 제대로 포맷")
4. CLAUDE.md 파일 전체에서 충돌하는 지침 찾기
...
프로젝트 루트 CLAUDE.md 는 압축을 완전히 생존: `/compact` 후 Claude 는 디스크에서 CLAUDE.md 를 다시 읽고 세션에 새로 다시 주입. 하위 디렉토리의 중첩된 CLAUDE.md 는 자동으로 다시 주입 안 됨 — 해당 하위 디렉토리 파일을 다시 읽을 때 다시 로드.

압축 후 사라진 것 같은 지침은:
- CLAUDE.md 에 작성되지 않고 대화에서만 제공된 경우
- 아직 다시 로드되지 않은 중첩된 CLAUDE.md 의 경우

세션 간 지속되도록 CLAUDE.md 에 대화 전용 지침 추가.
...
| 출력 결정 | 근거 |
|---|---|
| Root CLAUDE.md ≤ 200줄 목표 | "200줄 이하 목표 + 더 긴 파일은 준수 저하" |
| `.claude/rules/*.md` 생성 시 `paths:` glob 권장 | "조건부 로드 → 컨텍스트 절약" |
...
| 하위 디렉토리 CLAUDE.md 는 lazy load 됨을 사용자에게 통지 | "/compact 시 자동 재주입 안 됨 — 의존 시 주의" |
