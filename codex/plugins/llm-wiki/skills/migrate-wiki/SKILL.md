...
bridge_source: llm-wiki/migrate-wiki
---

# migrate-wiki

v1 deployments keep the wiki at `.codex/wiki/`, and `codex-bridge`'s `.codex/`→`.codex/` body transform forks a per-agent copy at `.codex/wiki/`. v2 introduces a neutral `.llmwiki/` root the transform can never touch. This skill migrates legacy layouts to v2 in place: detect, consolidate, add v2 frontmatter defaults, and additive-only grammar — without re-verifying or silent-deleting anything.

> Operates on the repo's wiki roots, resolved in order: `.llmwiki/wiki/` (preferred / target) →
> `.codex/wiki/` (legacy) → `.codex/wiki/` (legacy Codex fork). Migration moves legacy content
> into `.llmwiki/wiki/`.
...
- A repo has a legacy `.codex/wiki/` and you want the v2 `.llmwiki/` layout
- A repo has both `.codex/wiki/` and a forked `.codex/wiki/` that should be consolidated
...
1. **Detect**: report which wiki roots exist (`.llmwiki/wiki/`, `.codex/wiki/`, `.codex/wiki/`) and their schema state — do the pages already carry v2 frontmatter (`status:` / `volatility:` / `sources:`)? Summarize before changing anything.

2. **Consolidate (optional)**: copy per-agent legacy copies into `.llmwiki/wiki/`, deduping by `id`. On a conflict (same `id`, divergent bodies), present the divergence via `AskUserQuestion` and let the user pick — NEVER silent-delete. Keep the legacy dir (`.codex/wiki/` / `.codex/wiki/`) in place until the user explicitly confirms removal; do not auto-remove it.
...
| Delete / remove a legacy `.codex/wiki/` or `.codex/wiki/` dir | no | yes |
...
- .codex/wiki/   : <present/absent, N pages, v2-frontmatter? >
- .codex/wiki/    : <present/absent, N pages>

log.md entry appended FIRST:
## YYYY-MM-DD — migrate v1->v2 (migrate-wiki)
- <domain>/<page>.md: copied from .codex/wiki, +status:active +volatility:<x> +sources:N
- ...
...
Repo has `.codex/wiki/` (5 pages, no v2 frontmatter) and a forked `.codex/wiki/` (same 5 pages).
...
Detected:
- .llmwiki/wiki/  : absent
- .codex/wiki/   : present, 5 pages, no v2 frontmatter
- .codex/wiki/    : present, 5 pages (fork of .codex/wiki)

log.md entry appended FIRST:
## 2026-05-29 — migrate v1->v2 (migrate-wiki)
- design/layout.md: copied from .codex/wiki, +status:active +volatility:stable +sources:2
- backend/provider-x.md: copied from .codex/wiki, +status:active +volatility:volatile +sources:1
- backend/race-condition.md: copied from .codex/wiki, +status:active +volatility:volatile +sources:1
- code-map/api.md: copied from .codex/wiki, +status:active +volatility:stable +sources:0
- index.md: copied from .codex/wiki (no frontmatter change — index exempt)

Conflicts (needs user confirm): none (.codex/wiki was an exact fork)
Legacy dirs kept (not removed): .codex/wiki/, .codex/wiki/  (run removal only after user confirms)
```
...
- Legacy `.codex/wiki/` / `.codex/wiki/` dirs are still present (removal awaits user confirm)
...
- **Auto-removing `.codex/wiki/` / `.codex/wiki/`** — keep until the user explicitly confirms.
