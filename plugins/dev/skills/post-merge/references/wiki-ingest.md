...
This is the former `wiki:post-merge-wiki` skill, absorbed into post-merge as a required step. It reads what just merged and asks "what lore did we learn that the wiki should record?" — then delegates the heavy work (diff-log, multi-page cross-update, insight graduation) to `wiki:ingest-finding`.
...
- **No wiki root resolves** — wiki is a soft dependency; post-merge stays fully functional without it. Print `wiki-ingest: no-lore (no wiki root)`.
- **`wiki:ingest-finding` not installed** — note it as a manual follow-up. Print `wiki-ingest: no-lore (ingest-finding not installed)`.
...
For each candidate cleared to ingest (auto-ingested within-domain + any gated candidate the user accepted), invoke `/wiki:ingest-finding` (or run its steps inline if already loaded). The ingest skill handles the diff-log + cross-update discipline and applies v2 frontmatter defaults (`status: active`, inferred `volatility:`, `sources:`) to any new page.
