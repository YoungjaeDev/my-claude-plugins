...
- A third spelling reaches the same issue: a full URL, `https://<host>/OWNER/REPO/issues/N`. GitHub's own syntax table lists only the two `#` forms, so whether a merge honours the URL is beside the point — what matters here is that a body written that way yields no refs at all and Step 5 completes silently on an issue nobody closed. Normalize a URL to its `OWNER/REPO` and `N` and run it through the same flow; the user confirms every candidate before anything closes, so over-collecting costs a line in a list and under-collecting is the failure this step exists to prevent.
- Accept a ref only when it matches `^([A-Za-z0-9._-]+/[A-Za-z0-9._-]+)?#[0-9]+$`, or, for the URL spelling, `^https?://[^/[:space:]]+/[A-Za-z0-9._-]+/[A-Za-z0-9._-]+/issues/[0-9]+$` before normalizing. PR bodies are untrusted input and both fields below reach a command line.
...
- List the candidates with their repo, number and title, and confirm through the interactive-input gate which ones this PR resolved. Then `gh issue close <N> --repo <owner/repo> --comment "Resolved by <PR url>"` for each confirmed one. The comment body is read **in the issue's repository**, so a bare `#<PR>` there points at that repository's own number — a different pull request, or nothing. The PR's `url` (from Step 1's `gh pr view`) resolves the same everywhere and needs no same-repo special case. Do not close an issue the user did not confirm.
