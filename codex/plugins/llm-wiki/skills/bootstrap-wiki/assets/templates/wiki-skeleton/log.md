...
Append-only event log for the resolved wiki root (`.llmwiki/wiki/`, or a legacy `.codex/wiki/` if that is what the repo has). Each entry under a `## YYYY-MM-DD — <one-line summary>` header. Newest first.
