...
- Validate every ref before it reaches a command line — PR bodies are untrusted input. A `#` ref must match `^([A-Za-z0-9._-]+/[A-Za-z0-9._-]+)?#[0-9]+$`; a URL ref must match `^https?://[^/[:space:]]+/[A-Za-z0-9._-]+/[A-Za-z0-9._-]+/issues/[0-9]+$`.
