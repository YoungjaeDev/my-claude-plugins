...
- **Wiki pointer** — emitted only when a knowledge root resolves in cwd (`.llmwiki` → legacy `.claude` → `.codex`). core-config installs globally, so a repo with no wiki must not be told to read one. The MOC is named only when `.llmwiki/wiki/` is actually present: a repo carrying `.llmwiki/insight/` alone gets an insight-only pointer, never a path to an index that does not exist.
