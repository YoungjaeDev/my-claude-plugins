...
- Issue the remaining `codex exec` calls (N, or N-1 after an anchor) as parallel Bash tool calls, each writing its own file. Every variant session carries `count: 1` — the variation lives in the prompts, so a session that generates the full `-n` count would bill N times the images asked for.
