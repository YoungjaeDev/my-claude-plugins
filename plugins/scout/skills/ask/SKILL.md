...
Deep query a GitHub repository using DeepWiki's AI-powered documentation. Invoke it as `/scout:ask` or let the model reach for it during conversational research.
...
See `references/ask-procedure.md` in this plugin's installed root for the full workflow, smart-query strategy, multi-repo patterns, and error handling table. Under Claude Code this resolves to `${CLAUDE_PLUGIN_ROOT}/references/ask-procedure.md`; under Codex (which does not export `CLAUDE_PLUGIN_ROOT`) the plugin cache directory exposes the same `references/` subdirectory next to `skills/` (`~/.codex/plugins/cache/<marketplace>/scout/<version>/references/`). This reference is a plugin-root file (shared with the `generate-llmstxt` skill), so a skill-level install carries it only when the whole plugin is installed.
