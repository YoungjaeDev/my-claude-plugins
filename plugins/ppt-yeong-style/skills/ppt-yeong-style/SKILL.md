...
4. **기술 전제는 추측 말고 확인 후 명시.** 도식·명령·UI는 실제 런타임 사실 근거. 사실확인은 **공식 docs·신뢰 출처**로(Claude 런타임이면 `claude-code-guide` 에이전트도 활용 — 미번들·타 런타임이면 공식 docs 직접 확인). 도구 자체의 `/help`는 명령 목록일 뿐이라 기능·UX·권한모드 검증 근거로 쓰지 말 것. 미확인은 `unverified`.
