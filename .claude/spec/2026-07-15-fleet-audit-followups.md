...
- **스코프 경계(surgical)**: In — `glow-background.md` `:189`, `:198` 두 줄 + 같은 black 값을 사용자에게 지시하는 나머지 두 표면(`SKILL.md:354`, `visual-patterns.md:160,312` — 여기만 고치면 메인 생성 경로가 여전히 glow를 가리는 값을 emit) + `slidev` 버전 범프. Out — 다른 CSS 값(glow 색/blur/transition 표 `:314-325`, "per spec — do not change"), global-bottom.vue z-index/레이어링, black 스니펫과 무관한 타 slidev 레퍼런스.
...
- **스코프 경계**: In — 위 절차 + 정정된 전제(`/compact` 온디맨드 가능)를 `core-config/CLAUDE.md` compact-matcher 문단에 한 줄 addendum("runtime-verified via /compact + --debug hooks: <date>, 또는 pending")으로 명시 **+ core-config PATCH 범프와 marketplace `metadata.version` 범프** (`plugins/<name>/` 하위는 문서만 바뀌어도 범프 대상 — AGENTS.md/plugin-versioning 규칙; 실행분 PR #150이 실제로 1.12.1 + 2.5.5로 범프), 또는 이슈 #119 미체크 기준 종료 코멘트. Out — 자동 훅-테스트 하네스 구축(단일 훅에 YAGNI), plugin.json/prompt_inject.sh 코드 변경(구성상 이미 정확).
