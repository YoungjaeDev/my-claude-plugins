...
- **레이아웃 분포 셀프 체크(md 소스 확정 시)**: layout 태그를 종류별로 카운트한 분포 표를 사용자 검토(파이프라인 (3))에 함께 제출한다. 하드 트리거 = **동일 layout 3장 연속** 또는 **텍스트형(two-col·checklist·table) 합이 전체의 50% 초과** → 재분산 후 다시 검토. 단일 유형 % 임계 하나로는 못 잡는다(실측: 13장 중 checklist 4 + two-col 4 = 각 31%뿐인데도 단조로움 지적) — 분포 표를 사람 눈에 통과시키는 것까지가 게이트다.
