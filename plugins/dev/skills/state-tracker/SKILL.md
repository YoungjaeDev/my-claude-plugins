...
> Ships with the `dev` plugin; install via marketplace. Operates on the current repo's `.claude/state/spec.json`.
...
This op is automatically called by `dev:post-merge` at the end of the merge workflow when `.claude/state/spec.json` exists.
...
| `complete <spec>` | ✅ (when `dev:post-merge` chains) | — |
