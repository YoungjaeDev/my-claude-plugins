...
if (cd "$TMP/foreign" && env -u CLAUDE_PLUGIN_ROOT CODEX_PLUGIN_CACHE="$TMP/none" bash "$RUN") 2>/dev/null; then die "should abort with no skill dir"; else pass "aborts when unresolved"; fi
