...
- **앱 UI(로그인·diff·권한·설정·세션 등)는 실물 캡처 또는 점선 placeholder만 — codex/AI 생성 절대 금지.** 실제 화면이 진실이고 버전 종속이라, 지어낸 UI는 **틀린 UX를 사실처럼 가르쳐 placeholder보다 나쁘다**(실측: cc-common P08 가짜 로그인·P20 가짜 diff = codex가 존재하지 않는 화면을 그려냄). "재현 예시" 칩으로도 정당화 금지 — 청중은 칩을 안 읽고 화면을 진짜로 받아들인다.
