...
- A missing Project, or a `gh` token without the `read:project` scope, skips **this sub-step only** — Step 5 above already ran. It gets no `steps[]` entry of its own: `state-envelope/v0` carries one entry per top-level step and folds sub-steps into the parent, and Step 5's own entry is now always `done`, so a skipped Project can no longer be misread as a skipped issue-close check. Report which condition fired in the run output; the envelope does not need it.
