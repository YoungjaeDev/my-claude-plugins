---
name: github-dev-post-merge
description: "Clean up branch and integrate PR learnings into config files"
bridge_source: github-dev/commands/post-merge
---

# Post-Merge Cleanup

Perform local branch cleanup and configuration updates after a PR has been merged. For worktree removal, use `/exit` with cleanup option. Follow project guidelines in `@AGENTS.md`.
...
5.5. **Sync Milestone Progress** (if issues have milestones):
   ...
   - If milestone exists:
     ...
     2. Load state file: `.codex/state/project-tracking-{slug}.json`
     ...
     8. Update milestone description:
        ```bash
        MILESTONE_NUMBER=$(cat .codex/state/project-tracking-${SLUG}.json | jq -r '.milestoneId')
        # Fallback if null:
...
5.7. **Update `.codex/state/spec.json`** (if present)
   - If `.codex/state/spec.json` exists at the repo root, find the entry in `in_progress` whose `linked.pr` matches the merged PR number (or `linked.issue` if no PR ref).
   ...
   - The schema and update mechanics are owned by the `spec-state:state-tracker` skill — invoke it (`$state-tracker complete <spec-path>`) rather than direct JSON edit if the plugin is installed; otherwise apply a direct JSON edit using the schema documented in `plugins/spec-state/skills/state-tracker/SKILL.md`.
   - Skip entirely if `.codex/state/` directory does not exist.

5.8. **Trigger wiki ingest (if a wiki layer exists)**
   - Resolve the wiki root in order: `.llmwiki/wiki/` → `.codex/wiki/` → `.codex/wiki/`. Skip silently if none resolves (llm-wiki not in use — no hard dependency).
   - Skip for trivial merges (typo / dep-bump / formatting — reuse `post-merge-wiki`'s own "Do NOT use" list).
   - Otherwise surface via `AskUserQuestion` whether to run `$post-merge-wiki` (that skill derives + gates the ingest candidates itself). Invoke on accept; else note as a manual follow-up.
...
6. **Integrate Learnings into Configuration Files**
   ...
   >
   > Normative docs (AGENTS.md, AGENTS.md, GEMINI.md, .codex/rules/*, Serena memory) hold the **current rules only**. Change history (provenance) is already preserved permanently in git commit messages, PR bodies, and GitHub blame -- do not duplicate it inside the docs.
   >
   ...
   >
   > Use only when the section MUST hold time-ordered bullets that cannot be absorbed into normative sections (e.g. CHANGELOG entries migrated into a AGENTS.md summary). Do NOT use as a blanket escape -- when in doubt, absorb the learning into a topical section and delete the bullet.
   >
   ...
   **Pre-Audit: Clean Existing Pollution First**
   ...
   1. Build the target file candidate list: `AGENTS.md`, `AGENTS.md`, `GEMINI.md`, `.codex/rules/*.md` -- whichever exist.
   ...
   **CRITICAL: Never append "Post-Merge Notes" sections.** All learnings must be woven into the existing document structure as if they were always there.
   ...
   - Check which configuration files exist:
     - `AGENTS.md` - Claude Code specific instructions
   ...
   - **Expected root config structure** (AGENTS.md, AGENTS.md, GEMINI.md):
     ...
     4. Modular Rules - `See @.codex/rules/[module].md` references
   ...
   - **Classification and Placement** (applies to all config files):
     ...
     | Module-specific rule | `.codex/rules/[module].md` | Update or propose creation |
   ...
   - **Modular Rule Files** (.codex/rules/*.md):
...
1. Build candidate list (files that exist):
   - `AGENTS.md`, `AGENTS.md`, `GEMINI.md` at repo root
   - One-level matches under `.codex/rules/*.md` (no recursion)
...
4. If at least one file exceeds 32000 chars:
   ...
   b. `AskUserQuestion`, header `Size audit`:
      - **Split with rules-forge:split** (Recommended) — invoke `$split --threshold 20` for each oversized file. Extracts topical sections to `.codex/rules/<topic>.md`, rewrites the root with `@import` references. Best when the file is bulky.
      ...
      - **Defer** — print `Run $split or $claude-md-improver later on: <files>` and continue.
...
8. **Update README.md (if needed)**
   ...
   2. Apply `$humanize` to the changed sections to remove AI-generated patterns
   3. Apply `$readme` guidelines (CRO best practices, structure, clarity)
...
9. **Commit Changes (Optional)**
   ...
   - Stage only modified files: `git add AGENTS.md AGENTS.md GEMINI.md README.md .serena/memories/ 2>/dev/null || true`

> Follow ~/.codex/AGENTS.md and project AGENTS.md.
