...
- PR #193 (`voice-prompt`, merged `81e67e8`) — the qualified-name scope wrinkle. A Codex P2 caught a non-eligible plugin's body advertising `skill_view("docs-forge:voice-prompt")`; confirmed against `scripts/install-skills.mjs` (`id: fm.name` at the skill-entry build, plus the globally-unique-name assert in `--selftest`) that a skill-unit install registers the bare name and mints no namespace.
