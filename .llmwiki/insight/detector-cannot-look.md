...
A read-only check that cannot evaluate an axis must say so, keep going, and never emit an all-clear for the part it could not inspect. The failure hides as success in five shapes, all observed in this repo across 5 PRs (the fifth inverts the symptom — a false block instead of a false clean — but has the same cause):
...
- `.llmwiki/wiki/plugin-ops/detector-cannot-look-vs-nothing-wrong.md` — the promoted source page (Modes 1-8, PRs #104/#106/#122/#153/#164/#167/#183).
