...
| Skill | Description |
|-------|-------------|
| `ppt-yeong-style` | 덱 유형·md 소스 규약·작성 원칙 12종·밀도 리듬(중간 강화 기본)·역할 기반 색·codex-image vs SVG 경계·공식 로고 fetch·실물 스크린샷 우선·윤문·렌더 QA. 진입점 = `SKILL.md`, 세부는 `references/` 4종 + `assets/injection-prompt.md` |
...
└── skills/ppt-yeong-style/
    ├── SKILL.md                    # 진입점: 언제쓰나 + 파이프라인 + md 규약 + 원칙 12 + 밀도 리듬
    ├── references/
    │   ├── color-typography.md     # 색·타이포 (역할 기반 팔레트 + 폰트 폴백 + 토큰 예)
    │   ├── images-and-pop.md       # codex vs SVG 경계 + fade 범위 + 마스코트/pop + 스크린샷 배치
    │   ├── icons-logos.md          # 공식 SVG fetch → 인라인 → 근접성 → 상표권
    │   └── ppt-master-and-qa.md    # ppt-master 7-step(참조) + 윤문 + 완료 QA + 실사용 주의
    └── assets/
