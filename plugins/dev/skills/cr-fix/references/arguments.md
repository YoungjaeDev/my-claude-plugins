...
| `--no-minor-stop` | OFF (soft-stop ON) | Disable the minor soft-stop. Default: from iter 2 on, a cycle that applied only low-severity fixes (no `high` reassessed, nothing deferred) ends with `final_state=minor_floor` instead of looping the low-value tail. Pass this to keep looping until the `applied==0 && deferred==0` convergence. The churn stop (`final_state=churn`) has no opt-out — a reviewer that only finds material on the loop's own commits has nothing left to say about the PR. Its previous-iteration axis applies to code only: prose rewrites its own lines every iteration, so a prose finding is judged by the outside-the-PR-diff axis alone and a prose loop that will not converge ends at `iteration_cap` instead. |
...
- Step 5 pre-flight runs at the top of every iter. When CR + Codex have both already arrived, Step 6 / 6b polling is **skipped entirely**. Set `CODEX_PREFLIGHT_TIMEOUT` (seconds) to change the Codex grace cap used by pre-flight (default 600).
