...
- **Non-portable tool** — the check's own binary can't run on this OS (`realpath -m` on BSD aborts under `set -e`), so its gate skips every item and the loop converges `final_state=clean` — auto-merge eligible. Invisible in an interactive shell where a `grep`→ugrep shim masks it, so re-verify under `env -i PATH=/usr/bin:/bin` — but only when every tool the script needs is in stock userland. If one is not (macOS keeps `jq` in the Homebrew prefix), that run dies on tool absence having verified nothing, and it still passes on Linux; there, keep `PATH` and assert instead that the tools under test are BSD builds — GNU answers `--version`, BSD refuses, so a success means Homebrew shadowed the system copies. Outside a detector the same missing tool hides in pipeline *position*: `$(… | md5sum | cut | head)` takes its status from `head`, so `set -e` never fires and the empty value ships as if computed.
