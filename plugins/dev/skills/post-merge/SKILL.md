...
- Extract issue refs from the PR body **and from every commit message on the PR** (`gh pr view <PR_NUMBER> --json commits`), using `Closes` / `Fixes` / `Resolves`, any case, with or without a colon. GitHub honours a closing keyword in a commit message too, and such a reference never appears in the body, so a body-only scan reports a clean Step 5 over an issue nobody closed. Merge both sources and deduplicate on host + repo + number before showing anything.
...
- A missing Project, or a `gh` token without the `read:project` scope, skips **this sub-step only** — Step 5 above already ran. The skip is silent to the user but still recorded: `record_step 5.1 skipped "no GitHub Project"`, `... "no read:project scope"`, or `... "issue not in the project"`, naming which condition fired.
