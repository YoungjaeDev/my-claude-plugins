...
A JSON array of Codex review ids (numbers) that have been **surfaced to the user** in some prior iter / run on this PR. Persisted in `.codex/state/cr-fix-${PR_NUM}.json`. Inherited from the prior session's archived state at Step 2.
