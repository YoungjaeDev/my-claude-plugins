...
bridge_source: spec-state/state-tracker
---

# state-tracker

`.codex/state/spec.json` is a one-file aggregate of the project's spec/issue/PR work pipeline. It exists so an LLM (or human) can answer "what's currently in flight, and against which spec?" with a single `Read` — no `find` over `.codex/spec/` plus per-file frontmatter parse.

> Ships with `spec-state` plugin; install via marketplace. Operates on the current repo's `.codex/state/spec.json`.

## SSOT relationship

| Source | Authority | When it wins |
|--------|-----------|--------------|
| `.codex/spec/*.md` frontmatter (`status:`) | **SSOT** | Always — the spec file is the truth |
| `.codex/state/spec.json` | **aggregate cache** | Faster lookup; if it conflicts with frontmatter, regenerate from frontmatter |
...
```json
{
  ...
  "in_progress": [
    {
      "spec": ".codex/spec/<YYYY-MM-DD>-<slug>.md",
...
1. If `.codex/state/spec.json` does not exist → print empty state (`{ "in_progress": [], "completed": [] }`) and inform the user. Do NOT auto-create; ask if they want `init`.
...
2. `find .codex/spec -maxdepth 1 -name '*.md' -not -name '_template.md'`.
...
4. Write `.codex/state/spec.json` with `schema: 1` and `updated_at` = now.
...
This op is automatically called by `github-dev:post-merge` at the end of the merge workflow when `.codex/state/spec.json` exists.
...
- **Auto-create `.codex/state/spec.json` without asking** — the user may not want state tracking in this repo. `read` returns empty + suggests `init` instead.
